...
const filtered = computed<Project[]>(() => {
  ...
  const titleStr = (t: Project['title']) => {
    ...
    if (t && typeof t === 'object' && 'en' in t && typeof (t as any).en === 'string') {
      return (t as any).en
    }
    return Object.values(t as Record<string, string>)[0] || ''
  }
  ...
  if (type.value !== 'all') {
    list = list.filter(p => p.type === type.value)
  }
  
  const searchTerm = (q.value || '').trim().toLowerCase()
  if (searchTerm) {
    list = list.filter(p => {
      const title = titleStr(p.title)
      const description = typeof p.desc === 'string' 
        ? p.desc 
        : (p.desc && typeof p.desc === 'object' && 'en' in p.desc) 
          ? (p.desc as { en?: string }).en || '' 
          : ''
      const stack = (p.stack || []).join(',')
      
      return `${title} ${description} ${stack}`.toLowerCase().includes(searchTerm)
    })
  }
...
const open = (p: Project) => {
  const idx = filtered.value.findIndex((x: Project) => x.slug === p.slug && x.type === p.type)
  console.log('Opening project:', p.slug, 'at index:', idx)
  if (idx === -1) return
...
function next() {
  const i = Math.min(filtered.value.length - 1, currentIndex.value + 1)
  currentIndex.value = i
  current.value = filtered.value[i]
}
function prev() {
...
.select {
  ...
  background: linear-gradient(to bottom, transparent, transparent) padding-box,
    url('data:image/svg+xml,%3Csvg width="16" height="16" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"%3E%3Cpath fill="%23aab2bf" d="M7 10l5 5 5-5z"/%3E%3C/svg%3E') no-repeat right 10px center / 16px 16px, var(--surface);
  color: var(--text);
  outline: none;
  min-width: 128px;
}
